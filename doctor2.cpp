...
int main(){
  ...
  // This is to fill the ip address and port number of doctor. Note I have used LOOPBACK ADDRESS
    ...
    cout<<"Phase 3: Doctor 2 has a static UDP port "<<udpport<<" and IP address "<<str<<endl;    
  ...
  while(1){// infinite loop begin
    ...
    cout<<"Phase 3: Doctor 2 receives the request from the patient with port number "<<port_cli_udp;
    ...
    for (int k=0; k<6; k+=2)
     {
        ...
        {   int n=sendto(udpSock,doc_file[k+1].c_str(),1024,0,(struct sockaddr *)&clientAddr,cli);
            if(n<0)
                ...
                cout<<"Phase 3: Doctor 2 has sent estimated price "<<doc_file[k+1]<<"$ to patient with port number "<<port_cli_udp<<"."<<endl;
        }
