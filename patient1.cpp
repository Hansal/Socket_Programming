...
int main() {
    ...
    vector<string> u1=readfile("patient1.txt");     
    ...
    cout<<"Phase 1: Patient 1 has TCP port number "<<port<<" and IP address "<<ip<<endl;
 
    char buffer[1024], buffer1[1024], apoint[1024];
    
    cout<<"Phase 1: Authentication request from Patient 1 with username ";
    ...
    cout<<"Phase 1: Patient 1 authentication result: "<<buffer1<<"."<<endl;
    ...
    if (str=="Failure")    
    {
     ...
     cout<<"Phase 2: The following appointments are available for patient 1:"<<endl<<apoint<<endl;
    ...
    while(dummy!=0){ // this while loop will ask the patient to enter the correct index number:
      ...
      if (dummy==1)            
      {
      cout<<"You have entered an invalid entry, Please try again."<<endl;// from the following appointments and press enter"<<endl<<apoint<<endl;
      }
    }
    ...
    n=write(s0,num,1024); //sending the appointment number to the server
     ...
     memset(buffer,0,1024);
      ...
      if (!flag)
        ...
        cout<<"Phase 2:The requested Appointment is available and reserved to patient 1.";
    ...
    vector<string> patient_insurance=readfile("patient1insurance.txt");
    ...
    // to get the port number of the client udp
     ...
     cout<<"Phase 3: Patient 1 has a dynamic UDP port number "<<cli_udpport<<" and IP address "<<ip<<endl; 
     cout<<"Phase 3: The cost estimation request from Patient 1 with insurance plan "<<patient_insurance[0];
    ...
    if (n<0)
    {   cout<<"error in sending username :"<<endl;
    exit(0);
    }
    ...
    cout<<"Phase 3: Patient 1 receives "<<buffer_udp<<"$ estimation cost from doctor with port number ";
